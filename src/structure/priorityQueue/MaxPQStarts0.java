 ...
 * 
 * This is a max-oriented pq, the greater number is on the top
 * 
 * @author wuxu
 * @time 2016��1��4��
 */
public class MaxPQStarts0<Key extends Comparable<Key>> implements Iterable<Key> {

	private Key[] data;
	private int N;

	public MaxPQStarts0(int capacity) {
	...
	private class HeapIterator implements Iterator<Key> {
		// create a new pq
		private MaxPQStarts0<Key> copy;

		// add all elements to copy of heap
		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			System.out.println(data.length + " ");
			copy = new MaxPQStarts0<Key>(data.length - 1);
			for (int i = 0; i < N; i++) {
				if (data[i] != null) {
					copy.insert(data[i]);
				}
	...
	public static void main(String[] args) {
		MaxPQStarts0<Integer> heap = new MaxPQStarts0<Integer>(4);
